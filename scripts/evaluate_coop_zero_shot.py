"""Evaluate cooperative submissions on unseen layout families (zero-shot).

Used by instructors to measure generalization: teams are trained by students
on known layouts, then tested on held-out map families.
"""
...
from marl_course.evaluation.coop import CoopTeam, default_coop_teams, run_coop_zero_shot_evaluation


def load_teams(submissions: Path | None) -> list[CoopTeam] | None:
    """Load coop teams whose metadata indicates coop kitchen compatibility."""
...
def main() -> None:
    """Run zero-shot evaluation and save held-out family score summary."""
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", type=Path, default=Path("configs/evaluate_coop_zero_shot.json"))
    ...
    result = run_coop_zero_shot_evaluation(
        teams,
        episodes=int(cli_or_config(args, config, "episodes", 1)),
